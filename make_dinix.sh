...
echo "[1/4] Создание файлов для Dinix-0.0.3..."
rm -f boot.asm kernel.d linker.ld dinix.bin dinix.iso
rm -rf isodir

# 1. Ассемблерный загрузчик
cat << 'EOL' > boot.asm
bits 32
MODULEALIGN equ 1<<0
MEMINFO     equ 1<<1
...
# 2. Ядро Dinix на D с функцией memset
cat << 'EOL' > kernel.d
module kernel;
...
echo "[2/4] Компиляция бинарника..."
nasm -f elf32 boot.asm -o boot.o
ldc2 -m32 -betterC -release -c kernel.d -of=kernel.o
ld -m elf_i386 -T linker.ld -o dinix.bin boot.o kernel.o

echo "[3/4] Сборка образа dinix.iso..."
mkdir -p isodir/boot/grub
cp dinix.bin isodir/boot/dinix.bin
cat << 'EOL' > isodir/boot/grub/grub.cfg
set timeout=0
set default=0
menuentry "Dinix-0.0.3" {
    multiboot /boot/dinix.bin
    boot
}
EOL
grub-mkrescue -o dinix.iso isodir

echo "[4/4] Очистка..."
rm -rf isodir boot.o kernel.o

echo "----------------------------------------"
echo " УСПЕШНО! Образ 'dinix.iso' готов к работе."
echo "----------------------------------------"
